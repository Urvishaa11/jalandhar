...
def simulate_attack():
    ...
    for i in range(num_requests):
        ...
        anomaly_data = {
            ...
            'timestamp': datetime.now().isoformat(),
            'url': html.escape(request_data['url']),
            'method': html.escape(request_data['method']),
            'src_ip': html.escape(request_data['src_ip']),
    ...
    try:
        ...
        if result['is_anomaly']:
            anomaly_data = {
                ...
                'method': html.escape(data.get('method', 'GET')),
                'src_ip': html.escape(data.get('src_ip', '0.0.0.0')),
                'confidence': result['confidence'],
                'threat_types': result['threat_types'],
                'blocked': True
            }
